...
const DonationForm: React.FC<DonationFormProps> = ({ onClose }) => {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const { data: { clientSecret } } = await axios.post('/api/donations/create-payment-intent', { amount: parseInt(amount, 10) * 100 });
        ...
        <Card>
            <CardContent>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    />
                    <CardElement />
                    <Button type="submit" color="primary" variant="contained" disabled={!stripe} style={{ margin: '20px 0' }}>
                        Donate
                    </Button>
                </form>
            </CardContent>
